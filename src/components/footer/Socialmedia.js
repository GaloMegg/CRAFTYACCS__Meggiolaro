const SocialMedia = () => {
    return (
        <div className="footer__socialMediaFlex">
            <div className="socialmedia">
                <p className="socialmedia-p" alt="nuestras redes">Nuestras redes:</p>
                <a href="https://www.instagram.com/craftyaccs/" className="socialmedia--links links">Instagram</a>
                <a href="https://pin.it/2ZP8ilo" alt="inicio" className="socialmedia--links links">Pinterest</a>
                <a href="https://fb.me/CraftyAccesorios" alt="inicio" className="socialmedia--links links">Facebook</a>
                <a href="https://drive.google.com/drive/folders/1sOr6qRwOu9E6YdlU5yvvu4AbYi4vKfU7?usp=sharing" className="socialmedia--links links">Catálogo  completo</a>
            </div>
        </div>
    )
}
export default SocialMedia
