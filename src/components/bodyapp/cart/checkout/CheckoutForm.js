...
const CheckoutForm = ({ ValidationName, ValidationSurname, ValidationPhone, ValidationDni, ValidationEmail, ValidationAddress, ValidationZip, userName, userSurname, userPhone, userDNI, userAddress, userEmail, userZip, }) => {
    return (
        <>
            ...
            <input type="text" name="zip" placeholder="Codigo Postal" className={userZip ? "form__try" : "notValid"} required onChange={ValidationZip} onBlur={ValidationZip} />
            <input type="text" name="dni" placeholder="DNI" className={userDNI ? "form__try" : "notValid"} required onChange={ValidationDni} onBlur={ValidationDni} />
